...
uint8_t CAN_Init(CAN_TypeDef* CANx, CAN_InitTypeDef* CAN_InitStruct)
{
  ...
  while ((CANx->MCR & CAN_MCR_RESET) != 0)
  {
    wait_ack++;
  }
...
/**
  ...
  * @return CAN_TxStatus_Ok if the CAN driver transmits the message,
...
/**
  ...
  * @param  CANx: where x can be 1 or 2 to select the CAN peripheral.
  * @return CAN_Sleep_Ok if sleep entered, CAN_Sleep_Failed otherwise.
  */
...
/**
  ...
  * @param  CANx: where x can be 1 or 2 to select the CAN peripheral.
  * @return CAN_WakeUp_Ok if sleep mode left, CAN_WakeUp_Failed otherwise.
  */
...
/**
  ...
  *            @arg CAN_IT_LEC: Last error code Interrupt
  *            @arg CAN_IT_ERR: Error Interrupt
  * @return The current state of CAN_IT (SET or RESET).
  */
...
* @return CAN_TypeDef*
***********************************************/
CAN_TypeDef  *CAN_GetInstancePtr(int portNum)
{
    switch(portNum){
        case CAN_1:
            return CAN1;
