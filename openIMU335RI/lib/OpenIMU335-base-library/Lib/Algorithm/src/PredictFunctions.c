 ...
 */
static uint8_t RLE_F[ROWS_IN_F][2] = 
    { {  (uint8_t)STATE_RX, (uint8_t)STATE_VX  },     // Row  0: cols 0,3
      {  (uint8_t)STATE_RY, (uint8_t)STATE_VY  },     // Row  1: cols 1,4
      {  (uint8_t)STATE_RZ, (uint8_t)STATE_VZ  },     // Row  2: cols 2,5
      {  (uint8_t)STATE_VX, (uint8_t)STATE_ABZ },     // Row  3: cols 3,6:9,13:15
      {  (uint8_t)STATE_VY, (uint8_t)STATE_ABZ },     // Row  4: cols 4,6:9,13:15
      {  (uint8_t)STATE_VZ, (uint8_t)STATE_ABZ },     // Row  5: cols 5,6:9,13:15
      {  (uint8_t)STATE_Q0, (uint8_t)STATE_WBZ },     // Row  6: cols 6:12
      {  (uint8_t)STATE_Q0, (uint8_t)STATE_WBZ },     // Row  7: cols 6:12
      {  (uint8_t)STATE_Q0, (uint8_t)STATE_WBZ },     // Row  8: cols 6:12
      {  (uint8_t)STATE_Q0, (uint8_t)STATE_WBZ },     // Row  9: cols 6:12
      { (uint8_t)STATE_WBX, (uint8_t)STATE_WBX },     // Row 10: cols 10
      { (uint8_t)STATE_WBY, (uint8_t)STATE_WBY },     // Row 11: cols 11
      { (uint8_t)STATE_WBZ, (uint8_t)STATE_WBZ },     // Row 12: cols 12
      { (uint8_t)STATE_ABX, (uint8_t)STATE_ABX },     // Row 13: cols 13
      { (uint8_t)STATE_ABY, (uint8_t)STATE_ABY },     // Row 14: cols 14
      { (uint8_t)STATE_ABZ, (uint8_t)STATE_ABZ } };   // Row 15: cols 15

/******************************************************************************
 * @brief Porpagate the states of the Kalman filter through time.
 * 
 *  
 *****************************************************************************/
static void PredictStateEstimate(void);

/******************************************************************************
 * @brief Propagate the state covariance matrix of the Kalman filter through time.
 * 
 *  
 *****************************************************************************/
static void PredictCovarianceEstimate(void);

/******************************************************************************
 * @brief Record the Phi and dQ matrix related to the state covariance matrix
 *        change in PredictCovarianceEstimate().
 * 
 *  
 *****************************************************************************/
...
/******************************************************************************
 * @brief Update the process Jacobian of the Kalman filter.
 * 
 *  
 *****************************************************************************/
static void UpdateProcessJacobian(void);

/******************************************************************************
 * @brief Update the process covariance matrix of the Kalman filter.
 * 
 *  
 *****************************************************************************/
static void UpdateProcessCovariance(void);

/******************************************************************************
 * @brief A first-order 0.25Hz low-pass filter to 1D data.
 * 
 * @param output    [out]   output pointer to the filtered value
 * @param input     [in]    input pointer to a new raw value
 *  
 *****************************************************************************/
static void FirstOrderLowPass(real* const output, real const input);
...
void EKF_PredictionStage(real filteredAccel[])
{
    ...
    PredictStateEstimate();        // x(k+1) = x(k) + f(x(k), u(k))
    PredictCovarianceEstimate();   // P = F*P*FTrans + Q
     ...
     */
    FirstOrderLowPass( &gAlgorithm.filteredYawRate,
    ...
    {
        magFieldVector[X_AXIS] = 0.0F;
        magFieldVector[Y_AXIS] = 0.0F;
        magFieldVector[Z_AXIS] = 0.0F;
    }
...
static void PredictStateEstimate(void)
{
    ...
    if( gAlgorithm.state > (uint8_t)LOW_GAIN_AHRS ) 
    {
        ...
        gKalmanFilter.linearAccel_B[X_AXIS] += ( gKalmanFilter.correctedAccel_B[X_AXIS] +
                                                 ( gKalmanFilter.R_BinN[Z_AXIS][X_AXIS] * (real)GRAVITY)
                                               ) * gAlgorithm.dt;
...
static void PredictCovarianceEstimate(void)
{
     ...
     */
    UpdateProcessJacobian();     // gKF.F  (16x16)
    UpdateProcessCovariance();   // gKF.Q  (16x16)

    // Update P from the P, F, and Q matrices: P = FxPxFTranspose + Q
    // 1) PxFTranspose is computed first
    static real PxFTranspose[ROWS_IN_P][ROWS_IN_F]; // Define variables that reside on the heap
    memset(PxFTranspose, 0, sizeof(PxFTranspose));
    for (rowNum = 0U; rowNum < (uint8_t)ROWS_IN_P; rowNum++) 
    {
        for (colNum = 0U; colNum < (uint8_t)ROWS_IN_F; colNum++)
        {
    ...
    for (rowNum = 0U; rowNum < (uint8_t)16; rowNum++)
    {
        for (colNum = rowNum; colNum < (uint8_t)16; colNum++)
        {
...
static void UpdateProcessCovariance(void)
{
    ...
    static real multiplier_Q_Sq;

    static BOOL initQ_HG = TRUE;
    static real Q_WB_theory[3] = {0.0F};
    ...
    if (initQ_HG == TRUE) 
    {
        ...
        Q_WB_theory[X_AXIS] = gKalmanFilter.Q[STATE_WBX];
        Q_WB_theory[Y_AXIS] = gKalmanFilter.Q[STATE_WBY];
        Q_WB_theory[Z_AXIS] = gKalmanFilter.Q[STATE_WBZ];
    ...
    if( gAlgorithm.state >= (uint8_t)LOW_GAIN_AHRS ) 
    {
        gKalmanFilter.Q[STATE_WBX] = gAlgorithm.coefOfReduceQ * Q_WB_theory[X_AXIS];
        gKalmanFilter.Q[STATE_WBY] = gAlgorithm.coefOfReduceQ * Q_WB_theory[Y_AXIS];
        gKalmanFilter.Q[STATE_WBZ] = gAlgorithm.coefOfReduceQ * Q_WB_theory[Z_AXIS];
    }
     ...
     */
    real const q0q0 = gKalmanFilter.quaternion_Past[Q0] * gKalmanFilter.quaternion_Past[Q0] * 0.99F;
    real const q0q1 = gKalmanFilter.quaternion_Past[Q0] * gKalmanFilter.quaternion_Past[Q1] * 0.99F;
    real const q0q2 = gKalmanFilter.quaternion_Past[Q0] * gKalmanFilter.quaternion_Past[Q2] * 0.99F;
    real const q0q3 = gKalmanFilter.quaternion_Past[Q0] * gKalmanFilter.quaternion_Past[Q3] * 0.99F;

    real const q1q1 = gKalmanFilter.quaternion_Past[Q1] * gKalmanFilter.quaternion_Past[Q1] * 0.99F;
    real const q1q2 = gKalmanFilter.quaternion_Past[Q1] * gKalmanFilter.quaternion_Past[Q2] * 0.99F;
    real const q1q3 = gKalmanFilter.quaternion_Past[Q1] * gKalmanFilter.quaternion_Past[Q3] * 0.99F;

    real const q2q2 = gKalmanFilter.quaternion_Past[Q2] * gKalmanFilter.quaternion_Past[Q2] * 0.99F;
    real const q2q3 = gKalmanFilter.quaternion_Past[Q2] * gKalmanFilter.quaternion_Past[Q3] * 0.99F;

    real const q3q3 = gKalmanFilter.quaternion_Past[Q3] * gKalmanFilter.quaternion_Past[Q3] * 0.99F;
...
void GenerateProcessCovariance(void)
{
     ...
     */
    gKalmanFilter.Q[STATE_VX] = 100.0F * dtSigAccelSq;//(real)1e-10;
    gKalmanFilter.Q[STATE_VY] = 100.0F * dtSigAccelSq;
    gKalmanFilter.Q[STATE_VZ] = 100.0F * dtSigAccelSq;
}
...
static void StateCovarianceChange()
{
    ...
    for (rowNum = 0U; rowNum < (uint8_t)16; rowNum++)
    {
        for (colNum = 0U; colNum < (uint8_t)16; colNum++)
        {
            for (multIndex = RLE_F[rowNum][0]; multIndex <= RLE_F[rowNum][1]; multIndex++)
            {
    ...
    memcpy(gKalmanFilter.phi, gKalmanFilter.deltaP_tmp, sizeof(gKalmanFilter.phi));
    ...
    for (rowNum = 0U; rowNum < (uint8_t)ROWS_IN_F; rowNum++)
    {
        for (colNum = rowNum; colNum < (uint8_t)ROWS_IN_F; colNum++)
        {
            for (multIndex = RLE_F[colNum][0]; multIndex <= RLE_F[colNum][1]; multIndex++)
            {
...
static void FirstOrderLowPass( real * const output, real const input )
{
    ...
    {
        *output = ( 0.984414127416097F * (*output) ) +
                  ( 0.007792936291952F * (input + inputPast) );
    } 
    else 
    {
        *output = ( 0.992176700177507F * (*output) ) +
                  ( 0.003911649911247F * (input + inputPast) );
    }
...
#ifdef UNIT_TEST
#include "wrappers_PredictFunctions.h"
#endif
