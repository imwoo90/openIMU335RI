...
// Local functions
/******************************************************************************
* @brief Accumulate accel, gyro and magnetic measurements. These accumulated
*        will then be averaged as initial conditions.
**
*  .
******************************************************************************/
static void AccumulateFieldVectors(void);

/******************************************************************************
* @brief Average the accumulated accel, gyro and magnetic measurements.
**
* @param pointsToAverage    [in]    Number of samples accumulated for averaging.
*  
******************************************************************************/
static void AverageFieldVectors(uint32_t const pointsToAverage);

/******************************************************************************
 * @brief Force the algorithm to enter the high gain AHRS mode.
 * 
 *  
 *****************************************************************************/
static void DropToHighGainAHRS(void);

/******************************************************************************
 * @brief Reset/initialize Kalman filter matrices.
 * 
 *  
 *****************************************************************************/
...
void StabilizeSystem(void)
{
    ...
    //   the EKF)
    gAlgorithm.stateTimer = gAlgorithm.stateTimer - 1U;
    ...
    if (gAlgorithm.stateTimer == 0U) {
        ...
        gAlgorithm.state      = (uint8_t)INITIALIZE_ATTITUDE;
    ...
    gAlgoStatus.bit.algorithmInit  = (uint32_t)TRUE;
}
...
void InitializeAttitude(void)
{
    ...
    if (gAlgorithm.stateTimer == 0U) {
        ...
        // Set new state and timer
        gAlgorithm.state      = (uint8_t)HIGH_GAIN_AHRS;
         ...
         */
        AverageFieldVectors(gAlgorithm.Duration.Initialize_Attitude);
        ...
        gAlgoStatus.bit.algorithmInit         = (uint32_t)FALSE;
        gAlgoStatus.bit.highGain              = (uint32_t)TRUE;
        gAlgoStatus.bit.attitudeOnlyAlgorithm = (uint32_t)TRUE;
    }
}
...
void HG_To_LG_Transition_Test(void)
{
    ...
    if (gAlgorithm.stateTimer == 0U) {
        ...
        gAlgoStatus.bit.highGain = (uint32_t)FALSE;
    }
}
...
void LG_To_INS_Transition_Test(void)
{
    ...
    if (gAlgorithm.stateTimer > 0U) {
        ...
        if ( gpsUsedInAlgorithm() && gEKFInput.gpsUpdate && gEKFInput.gpsFixType ) 
        {
            ...
            gAlgorithm.state = (uint8_t)INS_SOLUTION;

            gAlgoStatus.bit.attitudeOnlyAlgorithm = (uint32_t)FALSE;
...
void INS_To_AHRS_Transition_Test(void)
{
    ...
    {
        gAlgorithm.timeOfLastSufficientGPSVelocity = gEKFInput.itow;
    }
    /* Determine the length of time it has been since the system 'moved' --
     * only linear motion considered (rotations ignored).
     */
    uint32_t timeSinceRestBegan;
    if (gEKFInput.itow >= gAlgorithm.timeOfLastSufficientGPSVelocity)
    {
        timeSinceRestBegan = gEKFInput.itow - gAlgorithm.timeOfLastSufficientGPSVelocity;
    }
    else
    {
        timeSinceRestBegan = ( gEKFInput.itow + MAX_ITOW ) - gAlgorithm.timeOfLastSufficientGPSVelocity;
    }
    ...
    if ( timeSinceLastGoodGPSReading > gAlgorithm.Limit.maxGpsDropTime )
    {
         ...
         */
        gAlgorithm.state      = (uint8_t)LOW_GAIN_AHRS;                 // HIGH_GAIN_AHRS;
        ...
        gAlgoStatus.bit.highGain              = gAlgorithm.state == (uint8_t)HIGH_GAIN_AHRS;
        gAlgoStatus.bit.attitudeOnlyAlgorithm = (uint32_t)TRUE;
    }
}
...
{
    real mult = 1.0F;
    if (pointsToAverage > 0U)
    {
        mult = 1.0F / (real)pointsToAverage;
    }
...
static void ResetAlgorithm(void)
{
    ...
    float32_t const s1 = 1.0F;
    float32_t const s2 = 0.01F; // s1=0.01, s2=0.1, HG=160sec can make ini_pitch=40 stable ,DXG
    ...
    for (elemNum = X_AXIS; elemNum <= Z_AXIS; elemNum++)
    {
        ...
        {
            gKalmanFilter.rateBias_B[elemNum] = 0.0F;
        }

        gKalmanFilter.correctedRate_B[elemNum] = 0.0F;
    }
...
#ifdef UNIT_TEST
#include "wrappers_SelectState.h"
#endif
